...
function AuthScreen({
  ...
  title = "Checking Auth",
  description = "Please wait while we verify your session and prepare your dashboard access.",
...
export default function AuthGuard({ children }) {
  ...
  useEffect(() => {
    ...
    if (startedRef.current || isBootstrapping) return;
    ...
    const bootstrapAuth = async () => {
      try {
        ...
        if (!user) {
          throw new Error("User data dari /auth/me tidak ditemukan");
        }
    ...
    bootstrapAuth();
  }, [hasHydrated, accessToken, refreshToken, isBootstrapping, locale, router]);
  ...
  if (!hasHydrated) {
    return (
      <AuthScreen
        eyebrow="Secure Access"
        title="Checking Auth"
        description="Please wait while we verify your saved session and prepare your dashboard access."
        badgeText="Hydrating session..."
      />
    );
  }
  ...
  if (!accessToken && !refreshToken) {
    return (
      <AuthScreen
        ...
        description="Your session is unavailable or has expired. Redirecting you to the login page."
  ...
  if (isBootstrapping) {
    return (
      <AuthScreen
        ...
        description="We found your session data and are validating it before opening the dashboard."
        badgeText="Authenticating..."
      />
    );
  }
